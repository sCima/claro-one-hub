...
function slaInfo(t: QueueItem, cfg: SlaCfg) {
  ...
  const ratio = t.openedMinsAgo / target;
  if (ratio < 0.7) return { label: `No prazo · ${target - t.openedMinsAgo}min`, cls: 'text-green-600 dark:text-green-400', bad: false };
  if (ratio < 1)   return { label: `Em risco · ${Math.max(target - t.openedMinsAgo, 0)}min`, cls: 'text-amber-600 dark:text-amber-400', bad: false };
  return { label: `Violado · +${t.openedMinsAgo - target}min`, cls: 'text-claro', bad: true };
}
...
export function AdminPanel({ onExit }: Props) {
  ...
  const [tab, setTab] = useState<'dashboard' | 'console'>('dashboard');
  if (!adminRole) return <AdminGate onExit={onExit} />;
  if (adminRole === 'atendente') return <AdminConsole onExit={onExit} />;
  return (
    <div className="h-screen flex flex-col bg-warm-100 dark:bg-warm-900 text-warm-900 dark:text-warm-50">
      <AdminTopBar onExit={onExit} tab={tab} onTab={setTab} />
      {tab === 'dashboard' ? <ManagerDashboard /> : <AdminConsole onExit={onExit} embedded />}
    </div>
  );
}
...
/* ─── Top bar branded ────────────────────────────────────────────────────── */
function AdminTopBar({ onExit, tab, onTab }: Props & { tab?: 'dashboard' | 'console'; onTab?: (t: 'dashboard' | 'console') => void }) {
  const { dark, toggleDark, adminLogout, adminRole, adminName } = useClaroContext();
  return (
    <header className="h-14 shrink-0 bg-warm-900 text-white flex items-center px-5 gap-4">
      ...
      {onTab && (
        <div className="hidden md:flex bg-white/10 rounded-full p-1 text-[11px] font-bold ml-2">
          <button onClick={() => onTab('dashboard')} className={cx('px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-colors', tab === 'dashboard' ? 'bg-white text-warm-900' : 'text-white/60')}><LayoutDashboard size={12} /> Visão Gerencial</button>
          <button onClick={() => onTab('console')} className={cx('px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-colors', tab === 'console' ? 'bg-white text-warm-900' : 'text-white/60')}><Headset size={12} /> Console</button>
        </div>
      )}
...
function AdminConsole({ onExit, embedded }: Props & { embedded?: boolean }) {
  const { attendanceQueue, resolveQueueItem, updateTicketStatus, claraMetrics, slaConfig, anonymizeTicket } = useClaroContext();
  ...
  const filtered = useMemo(() => attendanceQueue.filter((q) => {
    ...
    return true;
  }), [attendanceQueue, statusFilter, prioFilter, chanFilter, search]);
          ...
          <div className="lg:col-span-8 grid lg:grid-cols-8 overflow-hidden">
            ...
            <aside className="lg:col-span-3 border-l border-warm-200/70 dark:border-warm-700 bg-white dark:bg-warm-800 overflow-y-auto p-4 space-y-5">
              <div className="rounded-2xl border border-warm-200/70 dark:border-warm-700 p-4">
                ...
                <p className={cx('text-sm font-black flex items-center gap-1.5', slaInfo(selected, slaConfig).cls)}>{slaInfo(selected, slaConfig).bad && <AlertTriangle size={14} />}{slaInfo(selected, slaConfig).label}</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  ...
                  <div className="bg-warm-50 dark:bg-warm-700/40 rounded-xl p-2.5"><p className="text-warm-400">Aberto há</p><p className="font-bold">{selected.openedMinsAgo}min</p></div>
                </div>
              </div>
              <div>
